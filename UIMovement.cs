...
namespace VRStandardAssets.Utils
{
    ...
    //Little mod to this unity code 
    // This class is used to move UI elements in ways that are
    // This scrip has been modified to so the "lookat camara" just rotates the UI in one axis by Jorest
    ...
    public class UIMovement : MonoBehaviour
    {
        ...
        [SerializeField] private float m_FollowSpeed = 3.0f;     // The speed with which the UI should follow the camera.
        ...
        private void Update()
        {
            ...
            if (m_LookatCamera)
            {
                //  m_UIElement.rotation = Quaternion.LookRotation(m_UIElement.position - m_Camera.position); // this is the original version
               // make the ui rotates in the y axis
                m_UIElement.eulerAngles = new Vector3(m_UIElement.eulerAngles.x, m_Camera.eulerAngles.y, m_UIElement.eulerAngles.z);// modified so it just follows the camara in one axis
            }
